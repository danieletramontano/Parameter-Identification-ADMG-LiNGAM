...
#for(ww in c(1:5)){ ##Uncomment to run simulations in sequence
  if(ww == 1){
    ...
    saveRDS(dc_sols,file="/Data/cycle_uniform_data_10_1.Rds")
  }
  if(ww == 2){
    set.seed(44)
    dc_sols = fixed_graph_experiments(df_params = df_params,
    ...
    dc_sols$graph = "3-Cycle"
    saveRDS(dc_sols,file="/Data/cycle_uniform_data_10_2.Rds")
  }
  
  if(ww == 3){
    set.seed(45)
    dc_sols = fixed_graph_experiments_el(df_params = df_params,
                                      g_bid = graph(edges = c(), directed = FALSE),
                                      g_dir = graph(edges = c(1, 2, 2, 3, 3, 1), directed = TRUE),
                                      distribution = "Laplace")
    
    dc_sols$graph = "3-Cycle"
    saveRDS(dc_sols,file="/Data/cycle_uniform_data_10_3.Rds")
  }
  if(ww == 4){
    set.seed(46)
    dc_sols = fixed_graph_experiments(df_params = df_params,
    ...
    dc_sols$graph = "3-Cycle"
    saveRDS(dc_sols,file="/Data/cycle_uniform_data_10_4.Rds")
  }
  ...
  if(ww == 5){
    ...
    dc_sols$graph = "3-Cycle"
    saveRDS(dc_sols,file="/Data/cycle_uniform_data_10_5.Rds")
  }
}